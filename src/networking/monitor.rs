...
use crate::networking::service::MonitorService;
...
pub fn monitor_process(targets: Vec<String>, show_net: bool) {
    ...
    'main_loop: loop {
        ...
        let buf = if show_net {
            prepare_wtn_view(&data, &targets, sort_ascending)
        } else {
            prepare_monitor_view(&data, &targets, &order, num_cpus, sort_ascending)
        };
